...
[ -r "$list" ] || { echo "run-all: $list missing" >&2; exit 1; }
while read -r repo || [ -n "$repo" ]; do [ -n "$repo" ] && [ "${repo#\#}" = "$repo" ] || continue; bash "$here/../../plugins/core/scripts/nightly.sh" "$repo" || rc=1; done < "$list"
exit $rc
