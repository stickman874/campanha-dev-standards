...
R=$PWD; T=$(mktemp -d)
for b in opencode claude codex; do
...
assert_eq true "$(jq '.enabledPlugins["codex@openai-codex"]' "$T/codex/.claude/settings.json")" "codex: codex plugin on"
assert_eq opus "$(jq -r .model "$T/codex/.claude/settings.json")" "codex: orchestrator opus"
[ ! -d "$T/codex/.opencode" ] && [ -f "$T/codex/.claude/agents/reviewer.md" ] && echo "  ok  codex: .claude/agents only" || { echo "  FAIL codex layout"; FAILS=$((FAILS+1)); }
rm -rf "$T"; finish
