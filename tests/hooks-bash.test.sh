...
deny  block-unsafe-bash.sh 'git commit -n -m x'
deny  block-unsafe-bash.sh 'git commit -an -m x'
deny  block-unsafe-bash.sh 'git commit --no-ver -m x'
deny  block-unsafe-bash.sh 'git config core.hookspath /dev/null'
deny  block-unsafe-bash.sh 'git config CORE.HOOKSPATH /dev/null'
deny  block-unsafe-bash.sh 'git push --force origin main'
deny  block-unsafe-bash.sh 'git push --force-with-lease origin main'
deny  block-unsafe-bash.sh 'git push -f origin main'
deny  block-unsafe-bash.sh 'git push origin +main'
deny  block-unsafe-bash.sh 'git push origin main:prod'
deny  block-unsafe-bash.sh 'git push origin prod'
allow block-unsafe-bash.sh 'git commit -m "docs: explain -n"'
allow block-unsafe-bash.sh 'git log -n 5'
allow block-unsafe-bash.sh 'git push origin feature/production-fix'
finish
