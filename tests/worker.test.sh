...
assert_contains "$out" '^a.t$' "substring claim not matched against a real path"
...
out=$(echo x | ws read "$W/repo" 2>&1); code=$?
assert_eq 0 "$code" "2 tool calls, no change: exit 0"
printf '%s' "$out" | grep -q 'wrote nothing' && { echo "  FAIL 2 tool calls treated as wrote-nothing"; FAILS=$((FAILS+1)); } || echo "  ok  2 tool calls below the wrote-nothing threshold"
