...
- `deepseek.sh run`: runs the task packet's `Test:` command after the worker finishes and, if it fails, sends the output back for one correction round; reports `tests: pass` or `FAIL after one correction round`. The worker still has no shell (the command comes from the caller). Values from the repo's `.env` files are redacted from test output before it reaches the model. Orchestrators were sending test-bearing tasks to Sonnet because the worker had no shell.
