...
while read -r from to; do
  ...
  out=$(codex exec -s read-only --ephemeral --skip-git-repo-check -m "$model" -c model_reasoning_effort="\"$effort\"" -o "$tmp" \
    "Adversarial code review of the between commits $from and $to in this repository (run: git diff $from $to — trees compared directly; read files for context). Look for correctness bugs, security holes (authorization, data exposure, injection, secrets), data loss, broken edge cases, changed behaviour without a test. Ignore style. List findings as '[high|medium|low] file:line - what breaks - minimal fix'. End your final message with exactly one line: 'VERDICT: approve' if nothing high, else 'VERDICT: block'." 2>&1)
  rc=$?
  echo "$out"
