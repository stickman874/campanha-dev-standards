...
- Gruntwork (scoped edits, searches): `core:worker` skill ({% if backend == 'opencode' %}DeepSeek on opencode go{% elif backend == 'codex' %}gpt-6-sol via the Codex plugin's `codex:codex-rescue` subagent{% else %}Haiku subagent `worker`{% endif %}); exit 3 → Sonnet subagent (`Agent` tool, `model: sonnet`). You run the tests, trimmed (`| tail -40`).
- Stuck: `core:rescue` skill ({% if backend == 'opencode' %}fresh DeepSeek session{% elif backend == 'codex' %}fresh gpt-6-sol session via `codex:codex-rescue`{% else %}Sonnet subagent `rescuer`{% endif %}); verify with `git diff` and the failing command before trusting "done".
...
- Usage: subagents run on Haiku by default (`CLAUDE_CODE_SUBAGENT_MODEL`); `model: sonnet` only for the worker fallback and visual checks. Never Agent Teams.{% if backend != 'claude' %} Do not enable the Codex plugin's review gate.{% endif %}
