...
- `core:deepseek-worker` skill: `scripts/deepseek.sh` hands scoped edits/searches to DeepSeek V4.1 Flash through a no-shell opencode agent (no model relays the task), runs the task's `Test:` command for it with one correction round and gives up at the first usage-limit error; falls back to Sonnet if opencode, the guard or DeepSeek is unavailable.
