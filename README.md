...
- Secrets: sandbox + `permissions.deny` in the settings template; opencode denies dotenv reads natively; Codex has no such rule, so its reviewer runs in a clean worktree without `.env` and worker/rescue rely on the prompt.
