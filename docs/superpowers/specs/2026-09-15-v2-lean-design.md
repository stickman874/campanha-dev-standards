...
| # | Decision | Replaces |
|---|---|---|
| D1 | **Enforcement lives in lefthook `pre-push`, for humans and agents alike.** Codex adversarial review runs inside the hook (`codex review --base`). No markers, no Claude-only push gate. No CI service (owner declined GitHub Actions). | `pre-push-gate.sh`, `.git/campanha/*` markers, marker carry-forward in doc-keeper |
