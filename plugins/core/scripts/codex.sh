...
case $agent in reviewer) sandbox=read-only;; *) sandbox=workspace-write;; esac
...
(cd "$repo" && exec ${SETSID:-} codex exec --json --ephemeral --ignore-user-config -m "${CODEX_MODEL:-gpt-6-sol}" -s "$sandbox" -c 'windows.sandbox="unelevated"' -o "$last" - < "$tmp/prompt") > "$events" 2> "$tmp/err" &
pid=$!
