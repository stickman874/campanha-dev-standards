...
# `backend: codex` to codex.sh, except the `docs` agent, which stays on Claude (Haiku).
...
"')}
case $backend in
  claude) exec bash "$(dirname "$0")/claude.sh" "$@";;
  codex)  [ "$agent" = docs ] && exec bash "$(dirname "$0")/claude.sh" "$@"; exec bash "$(dirname "$0")/codex.sh" "$@";;
esac
tmp=$(mktemp -d); pid=
...
promptfile="$tmp/prompt"; command -v cygpath >/dev/null && promptfile=$(cygpath -w "$promptfile")   # Git Bash: opencode.exe needs a Windows path, not POSIX
(cd "$repo" && exec ${SETSID:-} opencode run "see attached file for the task" --format json --agent "$agent" --dir "$repo" --auto ${model[@]+"${model[@]}"} --print-logs --log-level ERROR -f "$promptfile") > "$events" 2> "$tmp/err" &
pid=$!
