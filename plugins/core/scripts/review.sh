#!/usr/bin/env bash
# Adversarial review of pushed ranges by the project's read-only `reviewer` agent (DeepSeek on opencode go, Sonnet on the Claude backend, gpt-6-sol on the Codex backend).
