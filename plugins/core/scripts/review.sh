...
SENSITIVE='auth|session|permission|payment|stripe|billing|src/app/api/|actions|server/|migrations|prisma/schema|lefthook|\.claude/|\.opencode/|^scripts/|opencode\.json|mise\.toml|AGENTS\.md|CLAUDE\.md|\.review-paths|\.copier-answers\.yml'
...
explicit=${1:+1}
# The one JSON object with a "verdict" key and this run's nonce ($2) anywhere in $1 (prose, fences, pretty-printed): jq reads one value from each "{" and ignores what follows.
...
extract_verdict_json() {
  ...
  for off in $(printf '%s' "$text" | grep -ob '{' | cut -d: -f1); do
    json=$(printf '%s' "$text" | tail -c +$((off + 1)) | jq -cn --arg n "$2" 'input | select(type == "object" and has("verdict") and .nonce == $n)' 2>/dev/null) && [ -n "$json" ] || continue
...
while read -r from to; do
  ...
  start=$(date +%s)
  nonce=$(od -An -N8 -tx1 /dev/urandom | tr -d ' \n')   # unknown to the diff, so a verdict copied from it never matches
...
Output: one JSON object and nothing else, no code fences, carrying this run's nonce exactly:
{"nonce":"$nonce","verdict":"approve"|"block","findings":[{"severity":"high"|"medium"|"low","file":"path","line":123,"what":"...","fix":"..."}]}
...
EOF
  ...
  json=$(extract_verdict_json "$out" "$nonce"); jrc=$?
