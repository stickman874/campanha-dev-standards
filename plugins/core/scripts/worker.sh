#!/usr/bin/env bash
# Worker through the project's no-shell `worker` agent: DeepSeek on opencode (.opencode/agents/), Haiku on the Claude backend or gpt-6-sol on the Codex backend (.claude/agents/).
...
# Exit 0 done · 3 DEEPSEEK_UNAVAILABLE / CLAUDE_UNAVAILABLE / CODEX_UNAVAILABLE (caller falls back to a Sonnet subagent) · 4 wrote nothing (≥3 tool calls, no change) · 2 bad usage.
set -u
...
[ "$code" -eq 0 ] || exit "$code"
tools=$(jq -c 'select(.type=="tool_use"), (select(.type=="assistant") | .message.content[]? | select(.type=="tool_use")), select(.type=="item.completed" and .item.type!="agent_message" and .item.type!="reasoning")' "$events" 2>/dev/null | wc -l)   # opencode events, Claude stream-json, Codex JSONL
...
claimed=$(printf '%s\n' "$out" | sed -n '/^Files:/,/^[A-Z][a-zA-Z]*:/p' | tr -d '`"'"'" | sed -n 's/^- \([^ ]*\).*/\1/p' | grep -v '^(' | sort -u)
paths=$(printf '%s\n' "$changed" | sed 's/^...//; s/.* -> //')
