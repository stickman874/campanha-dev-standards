---
name: review
description: Use when the pre-push review blocks, when the user asks for a review of a diff, or after writing a plan - a read-only reviewer judges (DeepSeek on opencode go, Sonnet on a Claude-only repo, gpt-6-sol on a Codex repo); you fix. Never bypass it yourself.
---
...
- Reviewer unavailable (no opencode/claude/codex, no login, quota): tell the user and stop.
